...
> then reveals it in an editor tab. The user saves it with <kbd>Ctrl</kbd>+<kbd>S</kbd>, choosing the location —
> we don't pick a path for them. Docs:
> [workspace.openTextDocument](https://code.visualstudio.com/api/references/vscode-api#workspace.openTextDocument).
> **Failure note:** it returns a `Thenable` — `await` it; and the doc is *untitled* until the user saves, so
...
> `Uri`s, or `undefined` if the user cancels — hence the `if (!uris?.length) return`. `canSelectMany: false`
