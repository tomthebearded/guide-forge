...
- **Starter combination** — one of the 5 base color sets (`bg` / `surface` / `text` / `accent1` / `accent2`) the reader picks as a starting point. — [glossary.md](../foundation/glossary.md)
...
the three moving parts: a **`StarterCombo`** (5 colors the user picks) goes into a profile's **`buildPalette`**,
