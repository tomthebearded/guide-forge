...
`isConfigured` flag to disable the button (and show a hint) when no client id is set, so if you haven't
pasted your Client ID you get a clear message instead of a broken redirect.
...
> update it to match or the browser tab will disagree with the on-screen brand. (`spotify-angular` stays the
