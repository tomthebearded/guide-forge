...
A learn-as-you-go guide that builds **spotify-trip**: a pure **Angular 21 (zoneless) + Angular Material 3 +
...
(project name `earthviewmusic`; `spotify-trip` is the guide's teaching name). The guide is reverse-engineered
...
```
examples/spotify-trip/
├─ TOKEN_USAGE.md                      # metered cost ledger (project level; the hook writes it)
└─ guide/
   ├─ README.md                        # thin front door (§3)
   ├─ PLAN.md                          # ← this file
   ├─ token-usage.md                   # planning/drafting cost ledger (seeded from the line below)
...
its own tokens mid-run. `/scaffold-guide` seeds `guide/token-usage.md`'s first row from this line.*
