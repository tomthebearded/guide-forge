...
| 4.6 | When an action repeats, teach the bulk path — or say there isn't one | "Select the tile images and set Pixels Per Unit to `18`, Filter Mode to Point, Compression to None." — on a pack of **231** PNGs, with no count anywhere and nothing about how the Inspector treats a multi-selection, so the reader works file by file and one of them keeps its default. | "The pack ships **231** tile PNGs, all needing the same three settings. Click the first, **Shift**-click the last so the Inspector shows the whole selection, set the fields once and press **Apply** — Unity re-imports all 231." Where no bulk route exists, one clause says so *with* the count; the gate then counts the set rather than sampling one member (6.6). |
...
| 6.6 | A gate that samples one case cannot assert the class | A badge reading `text/bg contrast 4.88:1 AA` in green, measured from the editor pair alone, on a theme whose sidebar sat at 2.53:1 | `worst pair tab.inactive 4.56:1 AA` — the minimum across all seven pairs — plus a sweep printing `all AA true` over every generated theme |
| 6.7 | Gate at the hardest condition the build actually reaches | "**Done when:** landing on an enemy from above destroys it and bounces you upward." Performed as a gentle step-off it passes; a real jump sinks the player's feet `0.47` units past the taught `0.1` tolerance and costs a life. | "**Done when:** **from a full jump off the ledge above**, landing on the enemy destroys it and bounces you upward — and a side contact still costs a life", with the limit written down: stomps verified at `10.7`, `22.0` and `31.9` u/s. |
